      ...
      <AnimatePresence>
        ...
        {isTransitioning && (
            <motion.div 
                ...
                exit={{ opacity: 0 }}
            >
                <motion.div className="absolute inset-0 bg-nature-500" initial={{ opacity: 0 }} animate={{ opacity: 1 }} />
                <motion.div 
                ...
                >
                    ...
                    <motion.p className="text-white font-black text-2xl mt-4 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>Let's Go!</motion.p>
                </motion.div>
            </motion.div>
        )}
        ...
        {isPausedMode && (
            <motion.div className="fixed inset-0 z-[40] flex flex-col items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="relative z-10 flex flex-col items-center p-8 text-center">
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1.5, rotate: -10 }} transition={{ type: "spring", bounce: 0.5 }} className="mb-6">
                       <PersimmonMascot frozen={true} className="w-32 h-32" />
                    </motion.div>
                    <motion.h2 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-4xl font-black text-white mb-2 drop-shadow-md">Time Frozen!</motion.h2>
                    <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="text-blue-100 font-bold mb-10 text-lg max-w-xs">Don't stay cold for too long.</motion.p>
                    <motion.button
            ...
            <motion.div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <motion.div className="bg-white rounded-[2.5rem] p-6 w-full max-w-sm shadow-2xl relative overflow-hidden" initial={{ scale: 0.8, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.8, opacity: 0 }}>
                    <div className="flex flex-col items-center text-center">
                        ...
                        <p className="text-slate-500 text-sm font-medium mb-6">Switching modes will end your current session. What do you want to do?</p>
                        <div className="flex flex-col gap-3 w-full">
                            <button onClick={() => confirmSwitch('FINISH')} className="w-full bg-nature-500 text-white font-bold py-3 rounded-xl hover:bg-nature-600 transition-colors shadow-lg shadow-nature-200">Mark as Done & Switch</button>
                            <button onClick={() => confirmSwitch('ABANDON')} className="w-full bg-orange-100 text-orange-500 font-bold py-3 rounded-xl hover:bg-orange-200 transition-colors">Reset Timer</button>
                            <button onClick={() => setPendingMode(null)} className="w-full text-slate-400 font-bold py-2 hover:text-slate-600 transition-colors text-sm">Cancel</button>
                        </div>
                    </div>
                </motion.div>
            </motion.div>
        ...
        {completionType && (
            <motion.div className="fixed inset-0 z-[70] flex items-center justify-center p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <motion.div className="relative z-10 bg-white rounded-[3rem] p-8 w-full max-w-sm text-center shadow-2xl overflow-hidden" initial={{ scale: 0.5, y: 100 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ type: "spring", bounce: 0.5 }}>
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                        ...
                        <h2 className="text-3xl font-black text-slate-800 mb-2 leading-tight">{completionType === 'FOCUS_DONE' ? "Session Complete!" : "Break is Over!"}</h2>
                        {completionType === 'FOCUS_DONE' ? (
                            ...
                            <p className="text-slate-500 font-medium mb-8 px-4">Hope you feel refreshed. Ready to get back in the zone?</p>
                        )}
                        <button onClick={handleManualNext} className={`w-full font-black py-4 rounded-2xl text-lg shadow-lg transform active:scale-95 transition-all text-white ${completionType === 'FOCUS_DONE' ? 'bg-nature-500 hover:bg-nature-600 shadow-nature-200' : 'bg-blue-500 hover:bg-blue-600 shadow-blue-200'}`}>
                        ...
                        <button onClick={() => setCompletionType(null)} className="mt-4 text-sm font-bold text-slate-400 hover:text-slate-600">Close / Stay here</button>
                    </div>
                </motion.div>
            </motion.div>
        )}
          ...
          <div className="flex flex-col md:flex-row items-center justify-center md:justify-center h-full w-full px-6 pt-8 pb-28 md:pb-0 md:pt-0 gap-4 md:gap-16 max-w-5xl mx-auto relative">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <div className="flex flex-col items-center gap-1 mt-2">
                           ...
                           {timeLeft !== settings.durations[mode] * 60 && !isActive ? (
                               <button onClick={handleResetTimer} className="group flex items-center gap-1 text-white/50 hover:text-white transition-colors mt-2" title="Reset Timer">
                               ...
                               <span className="text-white/80 font-bold text-lg bg-white/10 px-5 py-2 rounded-full backdrop-blur-md">{isActive ? 'Keep Focusing' : 'Ready?'}</span>
                           )}
                        </div>
                      </div>
                  </CircularTimer>
             ...
             <div className="flex-none md:flex-1 flex flex-col items-center md:items-start justify-center md:justify-center z-20 w-full md:w-auto text-white space-y-4 md:space-y-8 md:pl-8">
                ...
                <div className="flex items-center gap-4 md:block text-left w-full md:w-auto justify-center md:justify-start px-4 md:px-0">
                   ...
                   <div>
                      <h2 className="text-3xl md:text-5xl font-black mb-1 md:mb-3 tracking-tight leading-none">{MODE_CONFIG[mode].title}</h2>
                      <div className="bg-white/10 border border-white/10 rounded-xl px-3 py-1.5 inline-flex items-center max-w-[200px] md:max-w-xs">
                         <p className="text-white text-sm md:text-base font-bold truncate">
                             {activeTaskId ? <span>{tasks.find(t => t.id === activeTaskId)?.title}</span> : "Select a task or just start."}
                         </p>
                      </div>
                   </div>
                </div>
