...
export const BottomNav: React.FC<BottomNavProps> = ({ currentTab, setTab, isDarkBg }) => {
  
  const navItems: { id: TabView; icon: React.ReactNode; label: string }[] = [
    {
      id: 'TIMER',
      label: 'Focus',
      icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    },
    {
      id: 'TASKS',
      label: 'Tasks',
  ...
  return (
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full px-6 max-w-[280px]">
      <div className={`
        flex items-center justify-between p-1.5 rounded-full shadow-2xl backdrop-blur-2xl transition-all duration-500 ring-1
        ${isDarkBg 
            ? 'bg-black/20 ring-white/10' // Mode Gelap (Timer)
            : 'bg-white/80 ring-white/40 shadow-nature-500/10' // Mode Terang (Menu Lain)
        }
      `}>
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`
                relative w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 active:scale-90 group
                ${isActive 
                  ? 'text-white' // Ikon putih saat aktif
                  : (isDarkBg ? 'text-white/50 hover:text-white' : 'text-slate-400 hover:text-slate-600')
                }
              `}
            >
              ...
              {isActive && (
                <div className={`
                  ...
                  ${isDarkBg 
                    ? 'bg-white/20 shadow-white/5' 
                    : 'bg-nature-500 shadow-nature-200'
                  }
                `}></div>
              )}
