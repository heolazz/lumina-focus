...
import { motion, AnimatePresence } from 'framer-motion'; // Tambah import animasi
...
export const Settings: React.FC<SettingsProps> = ({ settings, updateSettings, installApp, isInstallable, onLogout }) => {
  ...
  const handleLogoutClick = () => {
    setShowLogoutConfirm(true); // Tampilkan modal, jangan langsung logout
  };
  ...
  return (
    <div className="w-full h-full min-h-screen flex flex-col lg:flex-row pb-24 md:pb-0 relative">
        ...
        {showLogoutConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm font-sans">
             <motion.div
                ...
                className="bg-white rounded-[2.5rem] p-6 w-full max-w-xs shadow-2xl border border-slate-100 text-center"
             >
                {/* Ikon Sedih/Tanya Maskot Kecil */}
                ...
                <p className="text-slate-500 font-medium text-sm mb-6 leading-relaxed">
                   Are you sure you want to leave? Your focus progress is saved.
                </p>
                ...
                <div className="flex gap-3">
                   ...
                   <button 
                      ...
                      className="flex-1 py-3 rounded-xl font-bold text-white bg-orange-500 hover:bg-orange-600 shadow-lg shadow-orange-200 transition-colors"
                   >
                      Yes, Bye!
                   </button>
                </div>
         ...
         <div className="max-w-xl mx-auto space-y-10">
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${settings.durations[TimerMode.POMODORO] === min
                          ? 'bg-white border-white text-nature-500 shadow-xl scale-105 z-10' 
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="bg-white/10 backdrop-blur-sm rounded-[2rem] p-4 border border-white/20 flex flex-col items-center">
                  ...
                  <div className="flex items-center justify-between w-full px-2">
                     <button onClick={() => updateDuration(TimerMode.SHORT_BREAK, Math.max(1, settings.durations[TimerMode.SHORT_BREAK] - 1))} className="w-10 h-10 flex items-center justify-center bg-white/20 rounded-full text-white hover:bg-white/40 transition-colors font-bold text-xl">-</button>
                     <span className="text-3xl font-black text-white">{settings.durations[TimerMode.SHORT_BREAK]}</span>
                     <button onClick={() => updateDuration(TimerMode.SHORT_BREAK, settings.durations[TimerMode.SHORT_BREAK] + 1)} className="w-10 h-10 flex items-center justify-center bg-white/20 rounded-full text-white hover:bg-white/40 transition-colors font-bold text-xl">+</button>
                  </div>
               </div>
            ...
            <div className="space-y-4">
               <div className="bg-white/20 backdrop-blur-md rounded-[2rem] p-1 flex gap-1">
                  <button onClick={() => updateSettings({ autoStartPomos: !settings.autoStartPomos })} className={`flex-1 py-4 rounded-[1.8rem] flex items-center justify-center gap-3 transition-all ${settings.autoStartPomos ? 'bg-white shadow-lg' : 'hover:bg-white/10'}`}>
                  ...
                  <button onClick={() => updateSettings({ autoStartBreaks: !settings.autoStartBreaks })} className={`flex-1 py-4 rounded-[1.8rem] flex items-center justify-center gap-3 transition-all ${settings.autoStartBreaks ? 'bg-white shadow-lg' : 'hover:bg-white/10'}`}>
               ...
               <div className="bg-white/10 backdrop-blur rounded-[2rem] p-6 border border-white/10">
                  <div className="flex items-center justify-between mb-4">
                     ...
                     <div className="flex bg-black/20 rounded-xl p-1 gap-1">
                        {(['NONE', 'RAIN', 'BROWN'] as const).map(sound => (
                           <button key={sound} onClick={() => updateSettings({ whiteNoise: sound })} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${settings.whiteNoise === sound ? 'bg-white text-nature-600 shadow-sm' : 'text-white/60 hover:text-white'}`}>
                  ...
                  {settings.whiteNoise !== 'NONE' && (
                     <input type="range" min="0" max="100" value={settings.volume * 100} onChange={(e) => updateSettings({ volume: parseInt(e.target.value) / 100 })} className="w-full accent-white h-2 bg-black/20 rounded-lg appearance-none cursor-pointer" />
                  )}
               </div>
            </div>
            ...
            <div className="space-y-4 pb-8">
                {isInstallable && (
                   <div className="bg-white/10 backdrop-blur-sm rounded-[2rem] p-6 border border-white/20 flex items-center justify-between transition-all hover:bg-white/15">
                      ...
                      <button onClick={installApp} className="bg-white text-nature-600 px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-slate-100 transition-all active:scale-95 flex items-center gap-2">
                ...
                {onLogout && (
                   <button 
                      ...
                      className="w-full bg-orange-100/10 border border-orange-200/30 hover:bg-orange-400 hover:border-orange-500 text-orange-100 hover:text-white rounded-[2rem] p-6 font-bold transition-all duration-300 flex items-center justify-center gap-3 group"
                   >
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 group-hover:-translate-x-1 transition-transform"><path fillRule="evenodd" d="M7.5 3.75A1.5 1.5 0 006 5.25v13.5a1.5 1.5 0 001.5 1.5h6a1.5 1.5 0 001.5-1.5V15a.75.75 0 011.5 0v3.75a3 3 0 01-3 3h-6a3 3 0 01-3-3V5.25a3 3 0 013-3h6a3 3 0 013 3V9A.75.75 0 0115 9V5.25a1.5 1.5 0 00-1.5-1.5h-6zm10.72 4.72a.75.75 0 011.06 0l3 3a.75.75 0 010 1.06l-3 3a.75.75 0 11-1.06-1.06l1.72-1.72H9a.75.75 0 010-1.5h10.94l-1.72-1.72a.75.75 0 010-1.06z" clipRule="evenodd" /></svg>
                      Sign Out
                   </button>
                )}
            </div>

            <div className="text-center text-white text-md font-bold pt-4">
