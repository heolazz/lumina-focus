...
interface SidebarProps {
  ...
  isDarkBg?: boolean; // Prop untuk mendeteksi apakah di atas background hijau
}

export const Sidebar: React.FC<SidebarProps> = ({ currentTab, setTab, isDarkBg }) => {
  ...
  // If Dark BG (Green): Glassy White
  // If Light BG (Cream): Glassy White with shadow or specific border
  
  // Style: Floating Dock on Left
  return (
    <div className="hidden md:flex fixed left-6 top-1/2 -translate-y-1/2 z-50">
      <div className={`
        flex flex-col items-center gap-6 py-8 px-3 rounded-[2.5rem] shadow-2xl backdrop-blur-xl border border-white/20 transition-all duration-500
        ${isDarkBg ? 'bg-white/10 text-white' : 'bg-white/80 text-slate-500 border-white/60'}
      `}>
        {navItems.map((item) => (
          <button
            key={item.id}
            ...
            className={`
              ...
              ${currentTab === item.id 
                ? (isDarkBg ? 'bg-white text-nature-600 shadow-lg' : 'bg-nature-500 text-white shadow-lg shadow-nature-200') 
                : 'hover:bg-white/20'
              }
            `}
            title={item.label}
          >
            ...
            {/* Tooltip on Hover */}
            <span className={`
                absolute left-full ml-4 px-3 py-1 rounded-lg text-xs font-bold shadow-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none
                ${isDarkBg ? 'bg-white text-nature-600' : 'bg-slate-800 text-white'}
            `}>
                {item.label}
            </span>
          </button>
